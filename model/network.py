...
from cfg.config import model_params, anchors
...
class Network(object):
    def __init__(self, inputs):
        ...
        self.output = self.decode(self.feature_maps, (self.grid_height, self.grid_width), self.class_num, self.anchors)
    ...
    def decode(self, feature_maps, output_sizes=(13, 13), num_class=3, anchors=None):
        """
        解码网络输出的特征图
        ...
        # 网络输出转化——偏移量、置信度、类别概率
        predict = tf.reshape(feature_maps, [-1, grid_H * grid_W, num_anchors, num_class + 5])
        ...
        # decode
        bbox_x = (x_cell + xy_offset[:, :, :, 0]) / grid_W
        bbox_y = (y_cell + xy_offset[:, :, :, 1]) / grid_H
        bbox_w = (anchors[:, 0] * wh_offset[:, :, :, 0]) / grid_W
        bbox_h = (anchors[:, 1] * wh_offset[:, :, :, 1]) / grid_H

        # 中心坐标+宽高box(x,y,w,h) -> xmin=x-w/2 -> 左上+右下box(xmin,ymin,xmax,ymax)
        boxes_xy = tf.stack([bbox_x, bbox_y], axis=3)
        boxes_wh = tf.stack([bbox_w, bbox_h], axis=3)

        return boxes_xy, boxes_wh, obj_probs, class_probs